...
VERSION_PACKAGE=github.com/ra1n6ow/miniblog/pkg/version
## 定义 VERSION 语义化版本号
...
ca: # 生成 CA 文件.
	...
	@# 3. 使用根私钥签发根证书 (CA CRT)，使其自签名，有效期为 10 年
	@openssl x509 -req -days 3650 -in $(OUTPUT_DIR)/cert/ca.csr -signkey $(OUTPUT_DIR)/cert/ca.key -out $(OUTPUT_DIR)/cert/ca.crt
	@# 4. 生成服务端私钥
	...
	@# 6. 创建扩展配置文件
	@echo '[v3_req]' > $(OUTPUT_DIR)/cert/extfile.cnf
	@echo 'subjectAltName=DNS:localhost,IP:127.0.0.1' >> $(OUTPUT_DIR)/cert/extfile.cnf
	@# 7. 使用根证书 (CA) 签发服务端证书 (Server CRT)，有效期为 1 年
	@openssl x509 -days 365 -sha256 -req -CA $(OUTPUT_DIR)/cert/ca.crt -CAkey $(OUTPUT_DIR)/cert/ca.key \
		...
		-extfile $(OUTPUT_DIR)/cert/extfile.cnf